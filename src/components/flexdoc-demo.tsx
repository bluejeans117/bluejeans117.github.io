...
import { ExternalLink, Maximize2, Minimize2, Play, Settings2, Sparkles, UploadCloud, Workflow } from 'lucide-react';
      ...
      <div className='container mx-auto max-w-[1500px] px-4 py-6 sm:py-10'>
          ...
          <div className='flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between'>
            <div className='max-w-4xl'>
              <div className='mb-3 inline-flex items-center gap-2 rounded-full border border-emerald-500/20 bg-emerald-500/10 px-3 py-1.5 text-xs font-semibold text-emerald-700 dark:text-emerald-300'>
                <Sparkles size={14} /> Live package: @prauga/flexdoc-client 2.8.0
              </div>
              <h1 className='text-2xl font-semibold tracking-[-0.03em] sm:text-4xl'>FlexDoc 2.8 full-surface API explorer</h1>
            ...
            <div className='grid w-full grid-cols-2 gap-2.5 sm:w-auto lg:flex lg:flex-col lg:items-end lg:gap-3'>
              <Link href='/flexdoc/playground' className='inline-flex min-w-0 items-center justify-center gap-2 rounded-xl border border-blue-500/25 bg-blue-500/10 px-3 py-2.5 text-center text-xs font-semibold text-blue-700 transition hover:bg-blue-500/15 sm:rounded-full sm:px-4 sm:py-2 sm:text-sm dark:text-blue-300'>
                <UploadCloud size={15} /> Try your own spec
              </Link>
              <Link href='/flexdoc/client' className='inline-flex min-w-0 items-center justify-center gap-2 rounded-xl border border-violet-500/25 bg-violet-500/10 px-3 py-2.5 text-center text-xs font-semibold text-violet-700 transition hover:bg-violet-500/15 sm:rounded-full sm:px-4 sm:py-2 sm:text-sm dark:text-violet-300'>
        ...
        <div ref={demoRef} className={`overflow-hidden rounded-2xl border shadow-2xl shadow-black/10 ${isFullscreen ? 'h-screen w-screen rounded-none border-0' : ''} ${isDark ? 'border-slate-700 bg-slate-900' : 'border-slate-200 bg-white'}`}>
          <div className={`flex flex-col gap-2 border-b px-4 py-3 sm:flex-row sm:items-center sm:justify-between ${isDark ? 'border-slate-700 bg-slate-900' : 'border-slate-200 bg-slate-50'}`}>
            <div className='flex min-w-0 items-center gap-3'>
              ...
              <span className='truncate text-xs text-foreground/55'>FlexDoc 2.8 product showcase — client 2.8.0</span>
            </div>
        ...
        <div className='mt-10 flex flex-col items-center justify-between gap-5 rounded-2xl border border-border bg-card/70 p-6 text-center sm:flex-row sm:text-left'>
          ...
          <div className='flex flex-wrap justify-center gap-3'>
            <Link href='/flexdoc/playground' className='inline-flex items-center gap-2 rounded-full border border-blue-500/25 px-4 py-2 text-sm font-semibold text-blue-700 transition hover:bg-blue-500/5 dark:text-blue-300'>
              <UploadCloud size={15} /> Upload a spec
            </Link>
            <Link href='/flexdoc/docs' className='inline-flex items-center gap-2 rounded-full border border-border px-4 py-2 text-sm font-semibold transition hover:border-blue-500/30'>
